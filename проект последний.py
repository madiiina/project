...
def startKBoard(message):
    ...
    startKBoard.row('reverse','bright','jpg to png')
...
def strong(message):
    ...
    bot.send_photo(message.chat.id, photo=open('image.jpg', "rb"))
def brightn(message):
    brightKBoard = telebot.types.ReplyKeyboardMarkup(True)
    brightKBoard.row('make bright','make dark','back')
    bot.send_message(message.chat.id, "Select the level of brightness:", reply_markup=brightKBoard)
def bright(message):
    mbrightKBoard = telebot.types.ReplyKeyboardMarkup(True)
    mbrightKBoard.row('light brightness','normal brightness','strong brightness')
    mbrightKBoard.row('back')
    bot.send_message(message.chat.id, "Select the level of brightness:", reply_markup=mbrightKBoard)
def dark(message):
    dbrightKBoard = telebot.types.ReplyKeyboardMarkup(True)
    dbrightKBoard.row('light dark','normal dark','strong dark')
    dbrightKBoard.row('back')
    bot.send_message(message.chat.id, "Select the level of dark:", reply_markup=dbrightKBoard)
def lbrightness (message):
    ...
    bot.send_message(message.chat.id, "Filter Brightness(light):")
    bot.send_photo(message.chat.id, photo=open('image.jpg', "rb"))
def nbrightness (message):
    for x in range(image.size[0]):
        for y in range(image.size[1]):
            r, g, b = image.getpixel((x, y))
            ...
            image.putpixel((x, y), (red, green, blue))
    image.save("image" + ".jpg")
    bot.send_message(message.chat.id, "Filter Brightness(normal):")
    bot.send_photo(message.chat.id, photo=open('image.jpg', "rb"))
def sbrightness (message):
    for x in range(image.size[0]):
        for y in range(image.size[1]):
            r, g, b = image.getpixel((x, y))
            ...
            image.putpixel((x, y), (red, green, blue))
    image.save("image" + ".jpg")
    bot.send_message(message.chat.id, "Filter Brightness(strong):")
    bot.send_photo(message.chat.id, photo=open('image.jpg', "rb"))
def ldark (message):
    for x in range(image.size[0]):
        for y in range(image.size[1]):
            ...
            image.putpixel((x, y), (red, green, blue))
    image.save("image" + ".jpg")
    bot.send_message(message.chat.id, "Filter Dark(light):")
    bot.send_photo(message.chat.id, photo=open('image.jpg', "rb"))
def ndark (message):
    for x in range(image.size[0]):
        for y in range(image.size[1]):
            ...
            image.putpixel((x, y), (red, green, blue))
    image.save("image" + ".jpg")
    bot.send_message(message.chat.id, "Filter Dark(normal):")
    bot.send_photo(message.chat.id, photo=open('image.jpg', "rb"))
def sdark (message):
    for x in range(image.size[0]):
        for y in range(image.size[1]):
            ...
            image.putpixel((x, y), (red, green, blue))
    image.save("image" + ".jpg")
    bot.send_message(message.chat.id, "Filter Dark(strong):")
    bot.send_photo(message.chat.id, photo=open('image.jpg', "rb"))
def rrotate (message):
    s=(128,128)
    m=image.thumbnail(s)
...
def send_text(message):
    ...
    if us == 'size':
        ...
        dark(message)
    elif us == 'light brightness':
        lbrightness(message)
    elif us == 'normal brightness':
        nbrightness(message)
    elif us == 'strong brightness':
        sbrightness(message)
    elif us == 'light dark':
        ldark(message)
    elif us == 'normal dark':
